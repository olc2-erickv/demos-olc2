...
class Aritmeticas(Operacion):
    ...
    def getValor2(self, driver, ts):
        ...
        if self.operador == Operador.SUMA:
            # INT + ?
            # FLOAT + ?
            if tipo_exp1 not in [Tipos.INT64, Tipos.FLOAT64]:
                # INT + INT
                # INT + FLOAT
                # FLOAT + INT
                # FLOAT + FLOAT
                if tipo_exp2 not in [Tipos.INT64, Tipos.FLOAT64]:
            ...
            if tipo_exp1 not in [Tipos.INT64, Tipos.FLOAT64]:
                if tipo_exp2 not in [Tipos.INT64, Tipos.FLOAT64]:
                    ...
                    print(f'La 2da expresion de la resta debe ser un integer o float ', self.exp2.linea,
                          self.exp2.columna)
            else:
